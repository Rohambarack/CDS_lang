...
def main():
    ...
    tracker = EmissionsTracker(project_name="A2_nn",
                            output_dir=a5_out,
                            output_file="emissions_a2_nn.csv")
    ...
    a2_analyse_nn = tracker.stop_task()
    ################################################
    #save output
    tracker.start_task("save")
    save_path = os.path.join("..","out")
    #classification report
    f = open(os.path.join(save_path,"cr_nn.txt"), 'w')
    f.write('Neural Network output\n\nClassification Report\n\n{}'.format(cr))
    f.close()
    #confusion mtrix
    cm_disp.plot()
    plt.savefig(os.path.join(save_path,"cm_nn.png"))

    from joblib import dump, load
    dump_path = os.path.join("..","models")
    dump(classifier, os.path.join(dump_path,"Nn.joblib"))
    dump(vectorisation, os.path.join(dump_path,"Vectorizer_Log_reg.joblib"))

    a2_save_nn = tracker.stop_task()
    #################################################
    _ = tracker.stop()
